...
# Primary Author: Cooper Sigrist
# Secondary Author: Fengkai Lin
...
def initialize(weights_size):
    ...
    w = np.zeros(weights_size) # dummy value
    b = 0 # dummy value
...
def predict(x: np.ndarray, w: np.ndarray, b: np.ndarray)->np.ndarray:
    ...
    prediction = np.dot(x, w) + b # Dummy value 
...
def Mean_square_error(x, y, w, b):
    ...
    loss = 0.5 * (predict(x, w, b) - y)**2 # Dummy value 
...
def loss_gradient(x, y, w, b, pred):
    ...
    d_w = np.dot((predict(x, w, b) - y), x)# Dummy value 
    d_b = (pred - y) # Dummy value 
...
def update(x, y, w, b, learning_rate):
    ...
    new_w, new_b = loss_gradient(x, y, w, b, predict(x, w, b))
    
    new_w = w - learning_rate * new_w
    new_b = b - learning_rate * new_b
...
def grad_descent(w, b, data_input_list, data_label_list, learning_rate):
    ...
    for num in range(len(data_input_list)):
        ...
        losses.append(Mean_square_error(x, y, w, b))
...
def main():
    ...
    X, Y = load_data("HW1\housing.csv")  # load the data set
